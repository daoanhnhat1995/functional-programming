...
(struct instruction (addr op dest))
    ...
    (cond
      ...
      ( (eq? exp 'x)
       ...
       (cond ; compile appropriately for "fall through" if branch is not taken
         ((= (+ 1 current) trueDest)
          (list (instruction current 'brF falseDest)))  ; trueDest is the next instruction
         (else
          (list (instruction current 'brT trueDest)))))
